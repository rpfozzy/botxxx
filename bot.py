...
def handle_message(message):
    try:
        ...
        if chat_type == 'private':
            process_private_message(user_id, user_text, message)
        elif chat_type in ['group', 'supergroup']:
            if user_text.startswith('.'):
                process_group_message(user_id, user_text, message)
            else:
                bot.reply_to(message, "В группах сообщения должны начинаться с точки (.)")
    except Exception as e:
        handle_error(e)

def process_private_message(user_id, user_text, message):
...
def process_group_message(user_id, user_text, message):
