...
# Запоминание сообщений для каждого пользователя
...
def handle_message(message):
    try:
        ...
        if user_id in user_messages:
            ...
            user_messages[user_id] = deque([f"пользователь:\n{message.text}\n"], maxlen=10)
        ...
        if any(keyword in user_text for keyword in ["рп", "ресурс пак", "топ", "пвп", "текстур пак"]):
            ...
            response_text = f"меня зовут фоззянка"
            bot.reply_to(message, response_text)
        elif user_text.startswith('.'):
            if user_id in special_users:
                gemini_response = get_gemini_response_special(user_id, user_text, special_users[user_id])
            else:
...
def get_gemini_response_special(user_id, question, special_message):
    previous_messages = ''.join(user_messages[user_id])
    combined_message = f"{previous_messages}----------------------------------------\n{question}\n\n{special_message}"
