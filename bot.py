...
# Маппинг эмодзи на стикеры
emoji_sticker_map = {
    '😈': 'CAACAgIAAxkBAAIJiV9jLBl6E8tdm03a4J6G3i1YmkmcAAJGAANWnbwKCowAI4pU2ssbBA',
    '🥵': 'CAACAgIAAxkBAAIJiV9jLBmvF9bL2q1LZtBYAfi0IjOQAAIhAANWnbwKJMX-NsRQMoAbBA',
    '🤔': 'CAACAgIAAxkBAAIJiV9jLBmESIZz4JpEF0FdKc56KgAB_wACGwADVo28ChX3gye4uH24GwQ',
    '😭': 'CAACAgIAAxkBAAIJiV9jLBmFJZkKekqEqSfoIqFgJtOwAAIuAANWnbwK0u-2FcT6xJUbBA',
    '🤩': 'CAACAgIAAxkBAAIJiV9jLBmlEWSVV6BbGDbEGB1k_c7TAAIfAANWnbwK4uQdKpji_BsbBA'
}
...
def handle_message(message):
    try:
        ...
        if any(keyword in user_text for keyword in ["рп", "ресурс пак", "топ", "пвп", "текстур пак"]):
            ...
            response_text = "меня зовут фоззянка"
            bot.reply_to(message, response_text)
...
if response.status_code == 200:
    ...
    return "извините, произошла ошибка при обработке запроса"def get_gemini_response_special(question, special_message): combined_message = f"{question}\n\n{special_message}"payload = {
    "contents": [{
        "parts": [{
            "text": combined_message
        }]
